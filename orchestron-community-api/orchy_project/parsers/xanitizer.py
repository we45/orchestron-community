...
def parse_xanitizer(xml_file,user_name,init_es):
	...
	for main in path :
		for finding in main.findall('finding'):
			...
			set_problemType = set(problemType_list)
			k = problemType in set_problemType
			problemType_list.append(problemType)
			if not k:
				try:
				...
				if classification == 'Must Fix':
					...
					severity_name =  'Info'
				evids=[]
				for finding_evd in main.findall('finding'):
					if problemType == finding_evd.findtext('problemType'):
						start_node =finding_evd.findall("startNode")
						classification = finding_evd.findtext("classification")
